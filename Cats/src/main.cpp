...
uint256 hashGenesisBlock("0x7da48374ca9bc5391e89f7d9f8b2d686ce2d4f8e63bac7e86a128df9bf6e8f8a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cats: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Cats Signed Message:\n";
...
{
    // Cats: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 500000); // Cats: 500k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 5 * 24 * 60 * 60; // Cats: 5 days
static const int64 nTargetSpacing = 3 * 60; // Cats: 3 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cats: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cats: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xd7438d74be64d9af4294606ee7e767e742885420b2d078fcdc67740953b4f06c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "11/23/2021 - CatsVision hits over a million vlews!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e11970d1f9dc44ab35d8b44679dff6e1acf008790a262725d28db61c80cb9c8520d807809e9b1c24eb145aa26796d4050d3d62afc33c7f127ef0bf046159752c")<< OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1637597305;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086723702;

        if (fTestNet)
        {
            block.nTime    = 1637597252;
            block.nNonce   = 385541874;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x871d3a6cd602734184b153ae442427f6b2b397cae4bbb1552976b53f33b383db"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb3, 0xda }; // Cats: increase each by adding 2 to catscoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CatsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CatsMiner(CWallet *pwallet)
{
    printf("CatsMiner started\n");
    ...
    try { loop {
        ...
        printf("Running CatsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CatsMiner terminated\n");
        throw;
    }
}

void Generatecatscoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CatsMiner, pwallet));
}
