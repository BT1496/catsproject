...
const int catscoin_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString catscoin_IPC_PREFIX("Cats:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(catscoin_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(catscoin_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(catscoin_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click catscoin: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start Cats: click-to-pay handler");
    else
...
{
    // clicking on catscoin: URLs creates FileOpen events on the Mac:
