...
void CWalletTx::RelayWalletTransaction()
{
    ...
    {
        // Important: versions of catscoin before 0.8.6 had a bug that inserted
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CcatscoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CcatscoinAddress(address).ToString());
}
